 ...
 public:
  ...
  void update(SDL_Rect& camera) {
    for (auto entity : getEntities()) {
      ...
      Logger::info("player: " + std::to_string(transformComp.pos.x) + " " + std::to_string(transformComp.pos.y));
      Logger::info("camera: " + std::to_string(camera.x) + " " + std::to_string(camera.y));
      if (transformComp.pos.x - (camera.w / 2) > 0 && (camera.w / 2) + transformComp.pos.x < Game::mapW) {
        camera.x = transformComp.pos.x - (camera.w / 2);
      }
      if (transformComp.pos.y - (camera.h / 2) > 0 && (camera.h / 2) + transformComp.pos.y < Game::mapH) {
